...
const VALID_GAMES = ["quiz", "tapissage", "geofela", "enquete"] as const;
const TOP_N = 5;
...
async function getTop(game: string) {
  return prisma.leaderboardEntry.findMany({
    where: { game, difficulty: null },
...
export async function GET(req: NextRequest) {
  ...
  const entries = await getTop(game);
  return NextResponse.json({ entries });
}

export async function POST(req: NextRequest) {
  let body: { game?: string; nickname?: string; score?: number };
  try {
    body = await req.json();
  } catch {
  ...
  const currentTop = await getTop(game);
  ...
  await prisma.leaderboardEntry.create({
    data: { game, difficulty: null, nickname: nickname.trim(), score },
  });
